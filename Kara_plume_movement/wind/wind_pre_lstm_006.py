...
def train_single_epoch(pretrained_encoder: torch.nn.Module,
    ...
    for (batch_data, next_day) in dataloader:
        optimizer.zero_grad()
...
def train_model(run_name: str,
    ...
    for epoch in range(max_epochs):
        train_loss = train_single_epoch(pretrained_encoder, lstm_network, decoder, optimizer, loss_function, dataset, batch_size=batch_size)
        
        test_loss = validate_single_epoch(pretrained_encoder, lstm_network, decoder, loss_function, dataset, batch_size=batch_size)
    ...
    torch.save(decoder, f'/app/Kara_plume_movement/wind/models/model_{run_name}_MLPdecoder.pth')
...
if __name__ == '__main__':
    ...
    decoder = MultiLayerPerceptron(input_size=dataset.num_days * pretrained_encoder.bottleneck,
                                   ...
                                   hidden_layers=[1024, 2024, 1024])

    pretrained_encoder = pretrained_encoder.cuda()
    lstm = lstm.cuda()
    decoder = decoder.cuda()

    run_name = 'wind_pre_lstm_run006'

    train_model(run_name, pretrained_encoder, lstm, decoder,
                dataset=dataset,
