...
def train_model(run_name: str,
    ...
    for epoch in range(max_epochs):
        ...
        if epoch == 64 | epoch == 192 | epoch == 448:
            torch.save(encoder, f'/app/Kara_plume_movement/wind/models/model_{run_name}_epoch_{epoch}_encoder.pth')
            torch.save(decoder, f'/app/Kara_plume_movement/wind/models/model_{run_name}_epoch_{epoch}_decoder.pth')
...
if __name__ == '__main__':
    ...
    encoder = Encoder(in_channels=2, H=41, W=201, expansions=[4, 4, 4, 4, 4], n_blocks=50, decreases=[2, 2, 2, 2, 2], bottleneck=64)
    decoder = Decoder(in_features=encoder.bottleneck, start_channels=2048, finish_channels=encoder.in_channels, n_layers=5,
                      expansion_value=0.25, increase_value=2, H=2, W=7, H_out=41, W_out=201)

    encoder = encoder.cuda()
    decoder = decoder.cuda()

    run_name = 'wind_pre_autoencoder_run006'
