...
def train_single_epoch(model: torch.nn.Module,
    ...
    for batch in dataloader:
        ...
        # 1) выбираем случайный timestep для каждого элемента батча
        t_idx = torch.randint(0, T, (N,), device=x.device)          # [N]
        n_idx = torch.arange(N, device=x.device)                    # [N]

        # 2) target: исходные вектора в замаскированных позициях
        target = x[n_idx, t_idx, :].clone()                         # [N, D]

        # 3) masked input: заменяем выбранные вектора на mask_value
        x_masked = x.clone()
        x_masked[n_idx, t_idx, :] = mask_value

        # 4) forward: модель должна вернуть предсказание замаскированного вектора
        pred = model(x_masked)                                      # [N, D]
...
def train_model(run_name: str,
    ...
    torch.save(model, f'/app/Kara_plume_movement/wind/models/model_{run_name}_encoder.pth')
...
if __name__ == '__main__':
    ...
    model = model.cuda()
    run_name = 'wind_Transformer1D_run001'
