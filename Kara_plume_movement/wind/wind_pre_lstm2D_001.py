...
def train_single_epoch(pretrained_encoder: torch.nn.Module,
    ...
    for (batch_data, next_day) in dataloader:
        ...
        decoded_lstm = lstm_decoder.forward(result_lstm)
        decoded_data = pretrained_decoder.forward(decoded_lstm)
...
if __name__ == '__main__':
    ...
    lstm_name = 'wind_pre_lstm_run002'
    ...
    lstm_network = torch.load(f'/app/Kara_plume_movement/wind/models/model_{lstm_name}_lstm_network.pth', map_location=torch.device('cpu'))
    lstm_decoder = torch.load(f'/app/Kara_plume_movement/wind/models/model_{lstm_name}_MLPdecoder.pth', map_location=torch.device('cpu'))
    ...
    run_name = 'wind_pre_lstm2D_run001'
