...
if __name__ == '__main__':
    wind_files_pkl = find_files('/mnt/hippocamp/asavin/data/wind/wind_arrays_kara_norm_n80_s70_w60_e90', '*.pkl')
    wind_files_pkl.sort()

    dataset = CustomDataset(wind_files_pkl, n_files=30)

    encoder = Encoder(in_channels=2, H=41, W=121, expansions=[4, 4, 4, 4], n_blocks=26, decreases=[2, 2, 2, 2], bottleneck=64)
    decoder = Decoder(in_features=encoder.bottleneck, start_channels=512, finish_channels=encoder.in_channels, n_layers=4,
                      expansion_value=0.25, increase_value=2, H=3, W=8, H_out=41, W_out=121)

    encoder = encoder.cuda()
    decoder = decoder.cuda()

    run_name = 'wind_pre_autoencoder_run002'
